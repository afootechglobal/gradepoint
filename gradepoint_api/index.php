...
break;
	...
	case 'add_or_update_faculty_api':
		$access_key=trim($_GET['access_key']);
			///////////auth/////////////////////////////////////////
			$fetch=$callclass->_validate_accesskey($conn,$access_key);
			$array = json_decode($fetch, true);
		  ...
		  if($check==0){ /// start if 0
			...
			$response['result']=False;
			  $response['message']='Invalid AccessToken. Please LogIn Again.'; 
				...
				$faculty_id=trim(strtoupper($_POST['faculty_id']));
				$faculty_name=trim(strtoupper($_POST['faculty_name']));

			
			
					if ($faculty_id==''){ ///start if 3
							///////////////////////geting sequence//////////////////////////
							$counter_id=2;
							$sequence=$callclass->_get_sequence_count($conn, $counter_id);
							$array = json_decode($sequence, true);
							...
							/// generate staff_id and password 
							$faculty_id='FAC'.$no.date("Ymdhis");
							
							/// register staff
							mysqli_query($conn,"INSERT INTO `faculty_tab`
							(`faculty_id`, `faculty_name`) VALUES
							('$faculty_id', '$faculty_name')")or die (mysqli_error($conn));
							
							$response['response']=504; 
							$response['result']=true;
							$response['message1']="SUCCESS!"; 
							$response['message2']="Faculty Registration Successful.";
							//$response['staff_id']=$staff_id; 
						
					}else{ ///else 3
							mysqli_query($conn,"UPDATE faculty_tab SET faculty_name='$faculty_name' WHERE faculty_id='$faculty_id' ")or die (mysqli_error($conn));
							$response['response']=506; 
							$response['result']=true;
							$response['message1']="SUCCESS!"; 
							$response['message2']="Faculty Updated Successful.";
	...
	case 'fetch_faculty_api':
	  ...
	  if ($check==0) { /// start if 0
			...
			$search_like="(faculty_name like '%$search_txt%')";
			...
			$count=mysqli_num_rows($query);
				...
				/// write sql statement and function that will return all staff here
                    if ($faculty_id=='') {///start if 	
						$query=mysqli_query($conn,"SELECT * FROM faculty_tab WHERE $search_like  ")or die (mysqli_error($conn));
						$check_query=mysqli_num_rows($query);
						if ($check_query>0) {
							$response['response']=603;
							$response['result']=true;
						...
						$query=mysqli_query($conn,"SELECT * FROM faculty_tab WHERE faculty_id='$faculty_id'  ")or die (mysqli_error($conn));
							$response['response']=605;
							$response['result']=true;
							while($fetch_query=mysqli_fetch_assoc($query)){
								$response['data']=$fetch_query;
	...
	case 'add_or_update_department_api':
		$access_key=trim($_GET['access_key']);
			///////////auth/////////////////////////////////////////
			$fetch=$callclass->_validate_accesskey($conn,$access_key);
			$array = json_decode($fetch, true);
			  ...
			  $response['message']='Invalid AccessToken. Please LogIn Again.'; 
				...
				$department_name=trim(strtoupper($_POST['department_name']));
					...
					if ($department_id==''){ ///start if 3
							...
							/// generate staff_id and password 
							$department_id='DEPT'.$no.date("Ymdhis");
							
							/// register staff
							mysqli_query($conn,"INSERT INTO `department_tab`
							(`department_id`,`faculty_id`, `department_name`) VALUES
							('$department_id','$faculty_id', '$department_name')")or die (mysqli_error($conn));
							
							$response['response']=504; 
							$response['result']=true;
							$response['message1']="SUCCESS!"; 
							$response['message2']="Department Registration Successful.";
							//$response['staff_id']=$staff_id; 
						
					}else{ ///else 3
							mysqli_query($conn,"UPDATE department_tab SET faculty_id='$faculty_id', department_name='$department_name' WHERE department_id='$department_id' ")or die (mysqli_error($conn));
							$response['response']=506; 
							$response['result']=true;
							$response['message1']="SUCCESS!"; 
							$response['message2']="Department Updated Successful.";
	...
	case 'fetch_department_api':
	  ...
	  if ($check==0) { /// start if 0
			...
			$search_like="(department_name like '%$search_txt%')";
			
			$query=mysqli_query($conn,"SELECT * FROM department_tab ")or die (mysqli_error($conn));
			$count=mysqli_num_rows($query);
				if ($count==0){///start if 1
					$response['response']=602;
					$response['result']=false;
				...
				/// write sql statement and function that will return all staff here
                    if ($department_id=='') {///start if 	
						...
						$query=mysqli_query($conn,"SELECT a.faculty_name, b.department_name FROM faculty_tab a, department_tab b WHERE a.faculty_id=b.faculty_id AND b.department_id='$department_id'  ")or die (mysqli_error($conn));
							$response['response']=605;
							$response['result']=true;
							while($fetch_query=mysqli_fetch_assoc($query)){
								$response['data']=$fetch_query;
	...
	case 'fetch_all_faculty_api':
	  ...
	  if($check==0){ /// start if 0
			...
			$query=mysqli_query($conn,"SELECT * FROM faculty_tab  ");
