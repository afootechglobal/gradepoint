...
$(document).ready(function(){
 ...
 function _expand_link(ids){
   ...
   function _get_active_link(divid){
		 $('#dashboard, #admin, #faculty, #department, #course,#gpa, #profile, #payment').removeClass('active-li');
...
function _semester(){
   ...
   if (semester==''){
      $('#nd1,#nd2').fadeOut(500);
   }else if (semester=='nd1'){
      $('#nd1').fadeIn(500);
   }else if (semester=='nd2'){
      $('#nd2').fadeIn(500);
   }
}
   ...
   function _get_page(page,divid){
      _get_active_link(divid);
         ...
         var dataString = 'action='+action+'&page='+page;
            ...
            success: function(html){
               $('#content').html(html);  
            // $('.overlay-div').html(html).fadeOut('fast');          
...
function _get_select_faculty(){  
 
   $.ajax({
   type: "POST",
   url: fetch_all_faculty_api,
   dataType: 'json',
   cache: false,
   success: function(info){
      var result= info.result;
    ...
    text='<option value="" >SELECT FACULTY </option>';
     ...
     if(result==true){
           for (var i = 0; i < fetch.length; i++) {
             ...
             /// for role update profile loop option ////
             text += '<option value="'+faculty_id+'">'+faculty_name.toUpperCase()+'</option>';
           }
         }else{
            text='<option value="" >'+message+' </option>';
         }
          $('#faculty_id').html(text);  //// for role registration option output /////
         // document.getElementById("updt_role_id").innerHTML = text;
   }
});
}
...
function _reg_faculty(faculty_id){
         var faculty_name=$('#faculty_name').val();
         $('#faculty_name').removeClass('complain');
      
         if(faculty_name==''){
            $('#faculty_name').addClass('complain');
            $('#warning-div').html('<div class="alert-logo"><img src="'+website_url+'/all-images/images/warning.gif" alt="Warning" /></div><h3>FACULTY NAME ERROR!</h3> <p>Fill this field to continue.</p></div').fadeIn(500).delay(3000).fadeOut(100); 
         }else{
            $('#faculty_name').removeClass('complain');
         ...
         var dataString ='faculty_id='+faculty_id+'&faculty_name='+faculty_name;
         $.ajax({
            type: "POST",
            url: add_or_update_faculty_api,
            dataType: 'json',
            data: dataString,
            cache: false,
            success: function(info){
               ...
               var message2 = info.message2;
                     ...
                     $('#success-div').html('<div class="alert-logo"><img src="'+website_url+'/all-images/images/success.gif" alt="Warning" /></div><h3>'+message1+'</h3> <p>'+message2+'</p></div>').fadeIn(500).delay(5000).fadeOut(100);
                     _alert_close();
                     _get_page('view_faculty','faculty');
...
function _get_fetch_faculty(faculty_id){  
   var dataString ='faculty_id='+faculty_id; 
      $.ajax({
         type: "POST",
         url: fetch_faculty_api,
         data: dataString,
         dataType: 'json',
         cache: false,
         success: function(info){
                  ...
                  if (result==true ){
                     for (var i = 0; i < fetch.length; i++) {
                      ...
                      text +=
                        ...
                        '<div class="insti-in">'+
                            '<h2>'+faculty_name+'</h2>'+
                        '<button class="btn" onclick="_get_form_page('+"'faculty_registration'"+','+"'"+faculty_id+"'"+')"><i class="bi-edit"></i> Edit </button>'+
                        '</div>'+
                    '</div>' ;
...
function _get_faculty_detail(faculty_id){
   var dataString ='faculty_id='+faculty_id;  
   $.ajax({
      type: "POST",
      url: fetch_faculty_api,
...
function _reg_department(deparment_id){
   var faculty_id=$('#faculty_id').val();
   var department_name=$('#department_name').val();
   $('#faculty_id, #department_name').removeClass('complain');

   if ((faculty_id=='') || (department_name=='')){
      $('#faculty_name, #department_name').addClass('complain');
      $('#warning-div').html('<div class="alert-logo"><img src="'+website_url+'/all-images/images/warning.gif" alt="Warning" /></div><h3>ERROR!</h3> <p>Fill all field to continue.</p></div').fadeIn(500).delay(3000).fadeOut(100); 
   }else{
      $('#faculty_id, #department_name').removeClass('complain');
   ...
   var dataString ='deparment_id='+deparment_id+'&faculty_id='+faculty_id+'&department_name='+department_name;
   $.ajax({
      type: "POST",
      url: add_or_update_department_api,
      dataType: 'json',
      data: dataString,
      cache: false,
      success: function(info){
         ...
         var message2 = info.message2;
               ...
               $('#success-div').html('<div class="alert-logo"><img src="'+website_url+'/all-images/images/success.gif" alt="Warning" /></div><h3>'+message1+'</h3> <p>'+message2+'</p></div>').fadeIn(500).delay(5000).fadeOut(100);
               _alert_close();
               _get_page('view_department','department');
...
function _get_fetch_department(department_id){  
var dataString ='department_id='+department_id; 
$.ajax({
   type: "POST",
   url: fetch_department_api,
   data: dataString,
   dataType: 'json',
   cache: false,
   success: function(info){
      ...
      var text = '';
            if (result==true ){
               for (var i = 0; i < fetch.length; i++) {
                ...
                text +=
                  ...
                  '<button class="btn" onclick="_get_form_page('+"'department_registration'"+','+"'"+department_id+"'"+')"><i class="bi-edit"></i> Edit </button>'+
                  '</div>'+
              '</div>' ;
            ...
            '<div class="no-record-div">'+
            '<span id="message">'+message+'</span><br clear="all">'+
            '<button class="btn"  onclick="_get_form_page('+"'department_registration'"+');"><i class="bi-person-plus"></i> ADD NEW FACULTY</button>'+
            '</div>';
            $('#fetch').html(text);    
         }
...
function _get_department_detail(deparment_id){
var dataString ='deparment_id='+deparment_id;  
$.ajax({
type: "POST",
url: fetch_department_api,
data: dataString,
